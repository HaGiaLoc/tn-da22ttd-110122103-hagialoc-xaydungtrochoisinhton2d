...
@onready var InventorySystem := get_node_or_null("/root/InventorySystem")
...
func _ready():
	...
	if id == "inventory" or String(id).strip_edges() == "":
		var base_name = get_owner().name if get_owner() else name
		id = "%s_%d" % [base_name, get_instance_id()]
		push_warning("InventoryModel: auto-generated id '%s' to avoid duplicate registrations." % id)
	...
	if config.autoload:
		if InventorySystem:
			...
			push_warning("InventoryModel (%s): config.autoload is true but InventorySystem singleton not found; skipping init()." % id)
...
func _inventory_view_input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		if held_item and held_item.vendor_item:
			var vendor_inventory = InventorySystem.get_inventory(held_item.parent_inventory)
			if not vendor_inventory or not vendor_inventory.has_method("buy_item"):
				return
			vendor_inventory.buy_item(held_item)
			return

func _on_slot_clicked(slot: InventorySlotUI, button: MouseButton, ctrl_pressed: bool, shift_pressed: bool) -> void:
	print("[InventoryModel] _on_slot_clicked: slot=%s button=%d has_item=%s is_holding=%s" % [
		slot.name, button,
		str(slot.get_inventory_item() != null),
		str(InventorySystem.is_holding_item())
	])
	if button == MOUSE_BUTTON_LEFT:
		...
		if vendor_inventory and vendor_inventory.has_method("sell_item") and vendor_inventory.visible:
			if ctrl_pressed:
...
func remove_item_at(slot_index: int) -> void:
	if items.has(slot_index):
		var slot: InventorySlotUI = inventory_view.get_child(slot_index)
...
func _unstack_item(slot: InventorySlotUI) -> void:
...
func _move_between_inventories(slot: InventorySlotUI) -> bool:
	var inventory_item: InventoryItem = slot.get_inventory_item()
	if not inventory_item:
		return false
...
func _add_item(inventory_item: InventoryItem, slot_index: int, with_affixes: bool = true) -> void:
	...
	var slot: InventorySlotUI = inventory_view.get_child(slot_index)
	print("[InventoryModel] _add_item: connecting hover on slot=%s mouse_filter=%d" % [slot.name, slot.mouse_filter])
	slot.mouse_entered.connect(InventorySystem.on_item_hover.bind(inventory_item, true))
	slot.mouse_exited.connect(InventorySystem.on_item_hover.bind(inventory_item, false))
