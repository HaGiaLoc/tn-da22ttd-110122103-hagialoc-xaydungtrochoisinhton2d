...
const AffixPool = preload("res://scripts/InventorySystem/itemization/AffixPool.gd")
var _affix_pool = AffixPool.new()
...
func roll_affixes():
	var candidates = _affix_pool.get_affixes_for(self)
	if candidates.is_empty():
		return
	
	var affix_count = randi_range(0, min(4, candidates.size()))
	for i in range(affix_count):
		var affix_instance = _affix_pool.roll_affix(candidates, self)
		if affix_instance:
...
func _calculate_worth():
	...
	for affix in affixes:
		var multi = _affix_pool.get_affix(affix.id).price_multiplier
