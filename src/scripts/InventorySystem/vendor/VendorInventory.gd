...
const VendorComponentScript = preload("res://scripts/InventorySystem/vendor/VendorComponent.gd")

var _vendor: VendorComponentScript
...
func _ready():
	if config:
		...
		push_warning("VendorInventory: no InventoryConfig assigned.")

	super._ready()

func set_vendor(vendor) -> void:
	clear()
	_vendor = vendor
...
func _on_slot_clicked(slot: InventorySlotUI, button: MouseButton, ctrl_pressed: bool, _shift_pressed: bool) -> void:
	if _place_back():
		accept_event()
		return
...
func buy_item(item: Item) -> bool:
	...
	if success:
		var currency_item_ref = item.currency_item  # lưu lại trước khi clear
		...
		if currency_item_ref:
			player_inventory.remove_item_by_id(currency_item_ref.id, cost)
		else:
